	   ...
	   */
		...
		for (let i = 0; i < lines.length; i++) 
		{
			...
			if (GROUP_RE.test(line)) 
			{
				...
				{
					groups.push(name);
					unpacked.groups.push(name);
				}
				currentGroupIndex = groups.indexOf(name);
			}
			...
			else if (TEXTURE_RE.test(line)) 
			{
				...
				if (elements.length > 2 && !options.enableWTextureCoord) {
					// coords = elements.slice(0, 1); << this is a BUG
				...
				*/
				if (elements.length > 4)	nonQuads++;
				else if (elements.length > 3)	quadCount++;
				else triangleCount++;
				...
				for (let j = 0, eleLen = elements.length; j < eleLen; j++) {
					...
					// ^^ there is a BUG in the quad-to-tri vertex ordering
					if (j === 3 && !quad) 
					{
					...
					const hash0 = elements[0] + "," + currentMaterialIndex +','+currentGroupIndex;
					const hash = elements[j] + "," + currentMaterialIndex +','+currentGroupIndex;
					if (hash in unpacked.hashindices) {
						...
						becomes:
							Vertex = ['19', '92', '11'];
						where
							idx[0] is the vertex index
							idx[1] is the texture index
							idx[2] is the normal index
						 Think of faces having Vertices which are comprised of the
						 attributes location (v), texture (vt), and normal (vn).
						 */
						let idx = elements[j].split("/");
						// it's possible for faces to only specify the vertex
						// and the normal. In this case, vertex will only have
						// a length of 2 and not 3 and the normal will be the
						// second item in the list with an index of 1.
						// let normalIndex = idx.length - 1;
						const normalIndex = idx.length > 2 ? 2 : -1;
						/*
						 ...
						 idx[2] will contain the one-based index of the vertexNormals
						 ...
						 */
						// Vertex position
						unpacked.verts.push(+verts[(idx[0] - 1) * 3 + 0]);
						unpacked.verts.push(+verts[(idx[0] - 1) * 3 + 1]);
						unpacked.verts.push(+verts[(idx[0] - 1) * 3 + 2]);
						// Vertex textures
						if (textures.length) {
							let stride = options.enableWTextureCoord ? 3 : 2;
							unpacked.textures.push(+textures[(idx[1] - 1) * stride + 0]);
							unpacked.textures.push(+textures[(idx[1] - 1) * stride + 1]);
							if (options.enableWTextureCoord) {
								unpacked.textures.push(+textures[(idx[1] - 1) * stride + 2]);
							}
						}
						if (normalIndex > -1)
						{
							// Vertex normals
							unpacked.norms.push(+vertNormals[(idx[normalIndex] - 1) * 3 + 0]);
							unpacked.norms.push(+vertNormals[(idx[normalIndex] - 1) * 3 + 1]);
							unpacked.norms.push(+vertNormals[(idx[normalIndex] - 1) * 3 + 2]);
						}
		...
		if (nonQuads)	log('found '+nonQuads+' faces with more than 4 vertices');
		if (quadCount)	log('found '+quadCount+' quad faces');
		if (triangleCount)	log('found '+triangleCount+' triangle faces');
		log('found '+unpacked.verts.length+' vertices');
		log(unpacked);
		self.vertices = unpacked.verts;
		self.normals = unpacked.norms.length ? unpacked.norms : null;
		self.uvs = unpacked.textures;
		self.materials = unpacked.materialIndices;
		self.indeces = options.indicesPerMaterial ? unpacked.indices : unpacked.indices[currentObjectByMaterialIndex];
