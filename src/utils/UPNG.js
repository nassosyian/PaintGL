...
function log() { if (typeof process=="undefined" || process.env.NODE_ENV=="development") log.apply(console, arguments);  }
...
UPNG.toRGBA16 = function(out)
{
	var w = out.width, h = out.height;
	if(out.tabs.acTL==null) return [UPNG.toRGBA16.decodeImage(out.data, w, h, out).buffer];
	...
	for(var i=0; i<out.frames.length; i++)
	{
		var frm = out.frames[i];
		var fx=frm.rect.x, fy=frm.rect.y, fw = frm.rect.width, fh = frm.rect.height;
		var fdata = UPNG.toRGBA16.decodeImage(frm.data, fw,fh, out);
...
UPNG.toRGBA16.decodeImage = function(data, w, h, out)
{
	var area = w*h, bpp = UPNG.decode._getBPP(out);
	var bpl = Math.ceil(w*bpp/8);	// bytes per line
	...
	if (ctype==6) { // RGB + alpha
		var qarea = area<<2;
		if(depth== 8) for(var i=0; i<qarea;i++) {  bf[i] = data[i];  /*if((i&3)==3 && data[i]!=0) bf[i]=255;*/ }
		// if(depth==16) for(var i=0; i<qarea;i++) {  bf[i] = data[i<<1] | data[(i<<1)+1];  }
		if(depth==16) bf = new Uint16Array(data.buffer);
	}
	else if(ctype==2) {	// RGB
		var ts=out.tabs["tRNS"], tr=-1, tg=-1, tb=-1;
		if(ts) {  tr=ts[0];  tg=ts[1];  tb=ts[2];  }
...
UPNG.encode.compress = function(bufs, w, h, ps, forGIF, forbidPlte)
{
	...
	//log("alpha check", Date.now()-time);  time = Date.now();
	
	var brute = gotAlpha && forGIF;		// brute : frames can only be copied, not "blended"
	var frms = UPNG.encode.framize(bufs, w, h, forGIF, brute);
	//log("framize", Date.now()-time);  time = Date.now();
	...
	if(ps!=0) {
		...
		//log("quantize", Date.now()-time);  time = Date.now();
	}
	else {
		...
		//log("make palette", Date.now()-time);  time = Date.now();
	}
	
	var cc=plte.length; //log("colors:",cc);
	...
	//log("colors => palette indices", Date.now()-time);  time = Date.now();
