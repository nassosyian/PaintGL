...
const FBXParser = (function () {
	...
	function parseGeometry( FBXTree, relationships, geometryNode, skeletons ) {
		...
		switch ( geometryNode.attrType ) {
			...
			default:
				log('ERROR: geomtry type "'+geometryNode.attrType+'" is not supported')
	...
	function parseMeshGeometry( FBXTree, relationships, geometryNode, skeletons ) {
		...
		// var preTransform = glMat4.identity([]);
		...
		if ( 'GeometricRotation' in modelNode.properties ) {
			...
			var array = modelNode.properties.GeometricRotation.value.map( _Math.degToRad );
			// var array = modelNode.properties.GeometricRotation.value.map( DEG_TO_RADIANS );
			array[ 3 ] = 'ZYX';

			preTransform.makeRotationFromEuler( new Euler().fromArray( array ) );
			// preTransform = glMat4.rotateZ(preTransform, preTransform, array[0]);
			// preTransform = glMat4.rotateY(preTransform, preTransform, array[1]);
			// preTransform = glMat4.rotateX(preTransform, preTransform, array[2]);
		}
		...
		if ( 'GeometricTranslation' in modelNode.properties ) {
			...
			// preTransform = glMat4.translate(preTransform, preTransform, modelNode.properties.GeometricTranslation.value)
		}
	...
	function genGeometry( FBXTree, relationships, geometryNode, skeleton, preTransform ) 
	{
		...
		vertexIndices.forEach( function ( vertexIndex, polygonVertexIndex ) {
			...
			hashKey = ''+vertexIndex;
			if (colorInfo)	hashKey += ','+getIndex(polygonVertexIndex, polygonIndex, vertexIndex, colorInfo)
			if (normalInfo)	hashKey += ','+getIndex(polygonVertexIndex, polygonIndex, vertexIndex, normalInfo)
			if (uvInfo)	hashKey += ','+getIndex(polygonVertexIndex, polygonIndex, vertexIndex, uvInfo[0])
			if ( materialInfo && materialInfo.mappingType !== 'AllSame' )	
				hashKey += ','+getIndex(polygonVertexIndex, polygonIndex, vertexIndex, materialInfo)

			// if ( hash.hasOwnProperty(hashKey)==false )
			if ( typeof hash[hashKey] === 'undefined' )
			{
				hash[hashKey] = hashCount;
				hashCount++;
				var idx = vertexIndex * 3;

				vertexBuffer.push( vertexPositions[idx + 0] )
				vertexBuffer.push( vertexPositions[idx + 1] )
				vertexBuffer.push( vertexPositions[idx + 2] )

				if ( colorInfo ) 
				{
					var data = getData( polygonVertexIndex, polygonIndex, vertexIndex, colorInfo );
					colorsBuffer.push( data[ 0 ], data[ 1 ], data[ 2 ] );
				}

				if ( normalInfo ) 
				{
					var data = getData( polygonVertexIndex, polygonIndex, vertexIndex, normalInfo );
					normalBuffer.push( data[ 0 ], data[ 1 ], data[ 2 ] );
				}
	
				if ( materialInfo && materialInfo.mappingType !== 'AllSame' ) 
				{
					materialIndexBuffer.push( getData( polygonVertexIndex, polygonIndex, vertexIndex, materialInfo )[ 0 ] );
				}
	
				if ( uvInfo ) 
				{
					var data = getData( polygonVertexIndex, polygonIndex, vertexIndex, uvInfo[0] );
	
					uvsBuffer.push( data[ 0 ] );
					uvsBuffer.push( data[ 1 ] );
				}
			}

			faceVertexIndices.push(hash[hashKey]);
			...
			if ( endOfFace ) {
				...
				{
					indexBuffer.push( faceVertexIndices[0] )
					indexBuffer.push( faceVertexIndices[ i-1 ] )
					indexBuffer.push( faceVertexIndices[ i ] )
				}
		...
		var geom = {
			name: sanitizeNodeName(geometryNode.attrName),
			indeces: indexBuffer,
			vertices: vertexBuffer,
			normals: normalBuffer,
			colors: colorsBuffer,
			uvs: uvsBuffer,
			materials: materialIndexBuffer,
			preTransform: preTransform,
			// vertexWeightsBuffer,
			// weightsIndicesBuffer,
		}
	...
	var GetIndex = {
		...
		ByPolygonVertex: {
			...
			Direct: function ( polygonVertexIndex, polygonIndex, vertexIndex, infoObject ) {return polygonVertexIndex;},
		...
		ByPolygon: {
			...
			Direct: function ( polygonVertexIndex, polygonIndex, vertexIndex, infoObject ) {return polygonIndex;},
		...
		ByVertice: {
			...
			Direct: function ( polygonVertexIndex, polygonIndex, vertexIndex, infoObject ) {return vertexIndex;}

		},

		AllSame: {

			IndexToDirect: function ( polygonVertexIndex, polygonIndex, vertexIndex, infoObject ) {return infoObject.indices[ 0 ];}
	...
	function getIndex( polygonVertexIndex, polygonIndex, vertexIndex, infoObject ) {

		return GetIndex[ infoObject.mappingType ][ infoObject.referenceType ]( polygonVertexIndex, polygonIndex, vertexIndex, infoObject );
	...
	function setModelTransforms( FBXTree, model, modelNode ) {
		...
		if ( 'Lcl_Rotation' in modelNode.properties ) {
			...
			var rotation = modelNode.properties.Lcl_Rotation.value.map( _Math.degToRad );
			rotation.push( 'ZYX' );
		...
		if ( 'PreRotation' in modelNode.properties ) {
			...
			var array = modelNode.properties.PreRotation.value.map( _Math.degToRad );
			array[ 3 ] = 'ZYX';
	...
	Object.assign( BinaryParser.prototype, {
		...
		parse: function ( buffer ) {
			...
			log( 'FBXLoader: FBX binary version: ' + version );
